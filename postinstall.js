...
function buildDarwin () {
  var lib = path.join(__dirname, 'deps/lib/libsodium.dylib')
  ...
  copy(lib, dst, function (err) {
    ...
    proc.exec('install_name_tool -id "@loader_path/libsodium.dylib" libsodium.dylib', {cwd: build}, function (err) {
      if (err) throw err
      proc.exec('install_name_tool -change "' + lib + '" "@loader_path/libsodium.dylib" huffin.node', {cwd: build}, function (err) {
        if (err) throw err
      })
    })
  })
